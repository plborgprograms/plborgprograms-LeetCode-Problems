//https://leetcode.com/problems/number-of-ways-to-reorder-array-to-get-same-bst/submissions/
class Solution {
public:
...
private:
    ...
    long long dfs(vector<int> &nums, long long mod){
        ...
        long long left_result = dfs(left, mod) % mod;
        long long right_result = dfs(right, mod) % mod;
		
		// look up table and multiple them together
		int left_length = left.size();
        return (((table[n - 1][left_length] * left_result) % mod) * right_result) % mod;
    }
};